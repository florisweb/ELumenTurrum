...
const String deviceId = "ELumen";
const String deviceKey = "theStromaIsQuiteLumious";
...
int targetMoistureValue1 = 40;


float volumePerWatering = 10; // mL
...
void onMessage(DynamicJsonDocument message) {
  ...
  } else if (packetType == "setTargetMoisture")
  {
    targetMoistureValue1 = message["data"];
    ConnectionManager.send("{\"targetMoistureValue1\": " + String(targetMoistureValue1) + "}");
  } else if (packetType == "measure")
  {
    sendSensorValues();
  }
}
...
void readSensorValues() {
  ...
  if (waterDetectorValue < 35) curVolume = minVolume;

  Serial.print("waterDetectorValue: ");
  Serial.println(waterDetectorValue);
...
void loop() {
  ...
  if (millis() - prevMillis > sensorUpdateFrequency)
  {
    prevMillis = millis();
    sendSensorValues();
    if (moistureValue1 < targetMoistureValue1) giveWater();
  }
}
